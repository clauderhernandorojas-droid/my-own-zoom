/**
 * WebLayoutOverrides.js — layout exclusivo navegador (panel flotante + chat oculto).
 * No se carga en Electron; si se incluye por error, exporta stubs vacíos.
 */
(function (global) {
  ...
  function setShellWebLayout(on) {
    const shell = deps?.$?.("roomShell");
    shell?.classList.toggle("room-shell--web-layout", !!on);
  }
  ...
  /** Evita que el panel flotante tape #roomRemoteScreenStage (logs: clics en centro → web-float-peers). */
