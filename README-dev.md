...
- **Base de datos**: Sequelize. Sin `DATABASE_URL` → **SQLite** en `data/app.sqlite`. Con `DATABASE_URL` → **PostgreSQL** con SSL (`rejectUnauthorized: false`), pool `{ max: 5, min: 0, acquire: 30000, idle: 10000 }` y `retry.max: 5` ([`src/config/database.js`](src/config/database.js)). Arranque: `server.listen` primero (health check Render); bootstrap de esquema (`authenticate` + `ensure*` + `sync`) en background vía [`src/services/dbBootstrap.js`](src/services/dbBootstrap.js) (`withDbRetry`); errores de conexión recuperables se loguean y **no** hacen `process.exit(1)`. Test: `npm run test:db-resilience`.
