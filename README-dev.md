...
| **Acciones rápidas con doble bucket** — La sección "Acciones rápidas" del lobby muestra dos listas independientes: `#scheduledList` (próximas, hasta 10, asc.) y `#scheduledPastList` dentro de un `<details>` colapsable (anteriores, hasta 10, desc.). Una reunión no aparece simultáneamente en ambos buckets: si tiene próxima ocurrencia futura va a "Próximas"; si no, su última ocurrencia pasada (incluyendo finalizadas) va a "Anteriores". Ambos buckets renderizan el item con la misma función `buildScheduledItem(r, mode)` (cero duplicación), compartiendo las acciones Entrar, Asistencia, Agendamiento, Editar, Cupo, Eliminar y Copiar código con los guards de rol/owner. Límite total visible: 20 items DOM | `public/index.html` (`selectMeetingsForBucket`, `buildScheduledItem`, `renderBucket`, `renderScheduledMeetings`) |
