...
- **Chat en sala (`chat.js`, fase 1 + 2)**:
  ...
  - **Bus interno en `document`** (no Socket.io): `moj:chat:notify`, `moj:chat:read`. Reglas: no-leído si hilo distinto al activo **o** panel oculto; sin badge en mensajes propios.
  - **`index.html`**: glue de sala (socket, `loadParticipantsForRoom`, `setChatPanelHidden`, WebRTC); delega chat a `ChatModule`.
...
*Última actualización de este documento: mayo 2026 — DELETE real en cascada (`eliminarReunionEnBd` en `src/services/reunionDelete.js`); `GET /api/reuniones/mis` devuelve buckets `{ proximas, anteriores }` ≤10 vía `src/services/reunionesMisBuckets.js`; lista completa del calendario en `GET /api/reuniones/calendario`; `calendarController.js` carga ambos endpoints; frontend consume buckets del API (sin `selectMeetingsForBucket` en cliente). Fix reagendar 500: `PATCH` post-reagendar usa `order: [['actualizado_en','DESC']]` en `ReunionOcurrencia` (no `actualizadoEn`).*
