...
| **Acciones rápidas con doble bucket** — La sección "Acciones rápidas" del lobby muestra dos listas independientes: `#scheduledList` (próximas, hasta 10, asc.) y `#scheduledPastList` dentro de un `<details>` colapsable (anteriores, hasta 10, desc.). Una reunión no aparece simultáneamente en ambos buckets: si tiene próxima ocurrencia futura va a "Próximas"; si no, su última ocurrencia pasada (incluyendo finalizadas naturales) va a "Anteriores". Ambos buckets renderizan el item con `buildScheduledItem(r, mode)` (cero duplicación), compartiendo Entrar, Asistencia, Agendamiento, Editar, Cupo, Eliminar y Copiar código con guards de rol/owner. **Eliminar:** el API hace soft-delete (`estado: finalizada`) y `GET /api/reuniones/mis` sigue devolviendo la fila; por eso, tras DELETE OK el cliente registra la PK en `lobbyRemovedReunionIds` (sesión) y `selectMeetingsForBucket` / `isLobbyReunionRemoved` la excluyen de **Próximas** y **Anteriores** aunque `loadHomeMeetings()` recargue la lista. Alcance: solo listas de Acciones rápidas (el calendario no usa este tombstone). Límite visible: 20 items DOM | `public/index.html` (`lobbyRemovedReunionIds`, `isLobbyReunionRemoved`, `selectMeetingsForBucket`, `buildScheduledItem`, `renderBucket`, `renderScheduledMeetings`) |
| **Convención de timestamps en modelos Sequelize** — Todos los modelos en [`src/models/`](src/models/) usan `timestamps: true` con alias físicos `createdAt: 'creado_en'` y `updatedAt: 'actualizado_en'` (heredado del `define` global de [`src/config/database.js`](src/config/database.js) y declarado explícitamente en las options de cada modelo para autodocumentación). No se declaran `createdAt`/`updatedAt` como atributos manuales del modelo ni se usan hooks `beforeUpdate` para tocar `actualizado_en`: Sequelize lo hace solo al hacer `.save()` / `.update()`. Servicios y rutas no deben asignar manualmente esas columnas. **Importante para consultas:** al usar `createdAt: 'creado_en'`/`updatedAt: 'actualizado_en'` Sequelize renombra el atributo del modelo (de `createdAt`/`updatedAt` a `creado_en`/`actualizado_en`); por lo tanto las consultas que ordenan/filtran por timestamps deben usar los alias físicos (`order: [['creado_en','ASC']]`, no `'createdAt'`), de lo contrario Sequelize lo trata como columna literal y falla con `SQLITE_ERROR: no such column: Reunion.createdAt` | Modelos: [`reunion.js`](src/models/reunion.js), [`reunionInvitado.js`](src/models/reunionInvitado.js), [`reunionSolicitudAcceso.js`](src/models/reunionSolicitudAcceso.js), [`reunionOcurrencia.js`](src/models/reunionOcurrencia.js), [`reunionAsistenciaMs.js`](src/models/reunionAsistenciaMs.js), etc. Config global: [`src/config/database.js`](src/config/database.js). Consumidor que aplica la convención al ordenar: [`src/services/reunionByRoom.js`](src/services/reunionByRoom.js) |
...
*Última actualización de este documento: mayo 2026 — Acciones rápidas con doble bucket (Próximas + Anteriores) en el lobby: `selectMeetingsForBucket(all, mode, now)` produce dos listas mutuamente excluyentes (10 ASC futuras / 10 DESC pasadas), `buildScheduledItem(r, mode)` reutiliza el mismo template con todas las acciones (Entrar, Asistencia, Agendamiento, Editar, Cupo, Eliminar, Copiar código) y `renderBucket(host, list, mode)` orquesta el render; el `<details id="scheduledPastWrap">` se oculta si no hay pasadas. Fix UX Eliminar en Anteriores: tombstone `lobbyRemovedReunionIds` + `isLobbyReunionRemoved` en `selectMeetingsForBucket` (el DELETE del API deja `finalizada` y `/mis` la reinyecta; sin tombstone el bucket past la mostraba otra vez tras `loadHomeMeetings`). Fix `SQLITE_ERROR: no such column: Reunion.createdAt` en `findReunionByRoomKey` (`src/services/reunionByRoom.js`): el `order` pasó de `['createdAt','ASC']` a `['creado_en','ASC']` porque, al alias `createdAt: 'creado_en'`, el atributo del modelo se renombra y `'createdAt'` ya no es válido en queries; convención unificada de timestamps automáticos en todos los modelos Sequelize: `timestamps: true` con alias `creado_en`/`actualizado_en` declarados explícitamente en las options de cada modelo, sin atributos manuales y sin hooks `beforeUpdate` redundantes (afecta `reunion.js`, `reunionInvitado.js`, `reunionSolicitudAcceso.js`, `reunionOcurrencia.js`, `reunionAsistenciaMs.js`; `upsertSessionMetrics` simplificado para que Sequelize llene timestamps); listado de reuniones por rol en `GET /api/reuniones/mis` (admin = todas, docente = solo las que creó, estudiante = vía `Participa`) implementado como cadena de capas (`src/utils/roles.js#getReunionScopeForUser` → `src/services/reunionesListing.js#listarReunionesParaUsuario` → `src/services/reunionPresenter.js#reunionJsonWithReagenda` → handler delgado en `src/routes/reuniones.js`), botón Eliminar del lobby restringido a `isOwner || isAdmin` con error visible y update optimista; Resize de trazos y resize de grupo en el tablero (`public/js/tableroSeleccion.js#getResizeTransform`, `applyResizeTransform` en `public/index.html`: 8 handles para stroke individual, bbox de unión + 8 handles para multiselección, Shift en esquina = uniforme, `lineWidth` y `fontSize` reescalan), multiselección (Shift+click, drag-box, drag/flechas/Delete agrupados), predicados de rol compartidos (`helpers.js`, `src/utils/roles.js`), panel admin (`/admin.html`, `/api/admin/usuarios`), Fase C persistencia `reunion_asistencia_ms`, Fase B métricas sesión RAM, Fase A métricas chat, reportes/exportación, asistencia en vivo opcional, calendario vertical, `calendarController.js` (paso 1), impresión lobby, modal de agenda, validación de solape, reagendar, historial deshacer/rehacer y reparación SQLite en arranque.*
